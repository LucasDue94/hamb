...
export class RedefineSenhaComponent implements OnInit {
  ...
  expireLink(id: number) {
    this.forgotService.get(id).subscribe(res => {

      const ONE_HOUR_EXPIRED = 3599999;
      this.today = new Date();
      this.redefineToDate = new Date(res.dateCreated);
      this.expiredHours = new Date(this.redefineToDate.getTime() + ONE_HOUR_EXPIRED);


      if (this.today > this.expiredHours || res.senhaAlterada) {
